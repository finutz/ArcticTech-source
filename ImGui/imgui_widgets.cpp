...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() ? c::accent : hovered ? c::scroll::i_bg_hov : c::scroll::i_bg, g.IO.DeltaTime * 6.f);
    ...
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, GetColorU32(it_anim->second.background), c::scroll::rounding);
...
void ImGui::Keybindbox(const char* label, const char* hint, bool* v, int* key, int* mode)
{

    std::string global_label = "1", keybind_label = label + global_label;
...
void ImGui::TooltipBind(const char* label, const char* hint, int* key, int* mode)
{
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items, float width)
{
    ImGuiContext& g = *GImGui;
    ...
    if (!BeginCombo(label, preview_value, items_count, width, ImGuiComboFlags_None))
        return false;
...
{
    const bool value_changed = Combo(label, current_item, Items_ArrayGetter, (void*)items, items_count, height_in_items, width);
    return value_changed;
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() || hovered ? c::input_text::i_bg_hov : c::input_text::i_bg, g.IO.DeltaTime * 6.f);

    if (!is_multiline)
    {
        GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(it_anim->second.background), c::input_text::rounding);
    }
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            ImU32 bg_color = GetColorU32(c::input_text::i_bg_selected); // FIXME: current code flow mandate that render_cursor is always true here, we are leaving the transparent one for tests.
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min, rect.Max, bg_color, c::input_text::rounding);
                }
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(total_bb.Min + ImVec2(it_anim->second.slow, 8), total_bb.GetBL() - ImVec2(-it_anim->second.slow, 8), GetColorU32(c::text::text_active));
    ...
    //if (label_size.x > 0)
  //      RenderText(ImVec2(frame_bb.Max.x - size_arg.x, frame_bb.Min.y - label_size.y - 8), label);

 //   GetWindowDrawList()->AddText(ImVec2(frame_bb.Max.x - size_arg.x, frame_bb.Min.y - label_size.y - 8), GetColorU32(c::text::text_active), label);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        PushStyleColor(ImGuiCol_PopupBg, GetColorU32(c::picker::i_bg));
        ...
        if (BeginPopup("picker"))
        {
            if (g.CurrentWindow->BeginCount == 1)
            {
                ...
                PushStyleColor(ImGuiCol_Text, GetColorU32(c::text::text));
                if (ImGui::Button(buf, ImVec2(GetContentRegionMax().x - style.WindowPadding.x, 30)));
                PopStyleColor();
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        PushStyleColor(ImGuiCol_Text, GetColorU32(c::text::text));
        ImGui::RenderTextClipped(window->DC.CursorPos + ImVec2(GetWindowContentRegionMax().x - 200, 0), window->DC.CursorPos + ImVec2(GetWindowContentRegionMax().x - 45, 22), buf, NULL, NULL, ImVec2(1.0f, 0.5f));
        PopStyleColor();
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    float square_sz = GetFrameHeight() + 5;
    ...
    float sv_bar_size = 17; // Saturation/Value picking box
    float bar0_pos_x = GetWindowPos().x + 15;
    ...
    // Render SV Square
    GetForegroundDrawList()->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), GetColorU32(ImGuiCol_PopupBg), col_white, hue_color32, hue_color32, col_white, c::picker::rounding, ImDrawFlags_RoundCornersTop);
    GetForegroundDrawList()->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), GetColorU32(ImGuiCol_PopupBg), 0, 0, col_black, col_black, c::picker::rounding, ImDrawFlags_RoundCornersTop);
    ...
    GetForegroundDrawList()->AddRectFilledMultiColorRounded(ImVec2(bar0_pos_x, bar_pos.y + 4), ImVec2(bar0_pos_x + bars_width, bar_pos.y + sv_bar_size - 5), GetColorU32(c::picker::i_bg), 0, 0, 0, 0, c::picker::rounding, ImDrawCornerFlags_All);

    float bar0_line_x = IM_ROUND(bar_pos.x + (1.f - H) * bars_width);

    bar0_line_x = ImClamp(bar0_line_x, picker_pos.x + 6.f, picker_pos.x + 204.f);

    it_anim->second.hue_bar = ImLerp(it_anim->second.hue_bar, bar0_line_x - bar_pos.x, g.IO.DeltaTime * 20.f);

    GetForegroundDrawList()->AddCircleFilled(ImVec2(it_anim->second.hue_bar + bar0_pos_x, bar_pos.y + 8.0f), 6, IM_COL32(255, 255, 255, 255), 100);
    ...
    if (alpha_bar)
    {
        ...
        ImRect bar1_bb(bar1_pos_x, bar_pos.y + 20, bar1_pos_x + bars_width, bar_pos.y + 25 + sv_bar_size);

        RenderColorRectWithAlphaCheckerboard(draw_list, bar1_bb.Min + ImVec2(0, 3), bar1_bb.Max - ImVec2(0, 10), 0, bar1_bb.GetWidth() / 50.0f, ImVec2(0.0f, 0.0f), c::picker::rounding);

        GetForegroundDrawList()->AddRectFilledMultiColor(bar1_bb.Min + ImVec2(0, 3), bar1_bb.Max - ImVec2(0, 10), user_col32_striped_of_alpha & ~IM_COL32_A_MASK, user_col32_striped_of_alpha, user_col32_striped_of_alpha, user_col32_striped_of_alpha & ~IM_COL32_A_MASK);
        GetForegroundDrawList()->AddRectFilledMultiColorRounded(bar1_bb.Min + ImVec2(0, 3), bar1_bb.Max - ImVec2(0, 10), GetColorU32(c::picker::i_bg), 0, 0, 0, 0, c::picker::rounding, ImDrawCornerFlags_All);
        float bar1_line_x = IM_ROUND(bar_pos.x + alpha * bars_width);

        bar1_line_x = ImClamp(bar1_line_x, bar_pos.x, picker_pos.x + 198.f);
        it_anim->second.alpha_bar = ImLerp(it_anim->second.alpha_bar, bar1_line_x - bar_pos.x + 5.f, g.IO.DeltaTime * 25.f);
        GetForegroundDrawList()->AddCircleFilled(ImVec2(it_anim->second.alpha_bar + bar_pos.x, bar1_bb.Min.y + 7.5f), 6, IM_COL32(255, 255, 255, 255), 100);
    }
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f)
    {
        ...
        window->DrawList->AddRectFilled(bb_inner.Min, ImVec2(mid_x, bb_inner.Max.y), GetColorU32(col_rgb_without_alpha), c::picker::rounding, ImDrawFlags_RoundCornersLeft);
    }
    else
    {
...
static ImGuiTabItem* ImGui::TabBarTabListPopupButton(ImGuiTabBar* tab_bar)
{
    ...
    PushStyleColor(ImGuiCol_Text, arrow_col);
    PushStyleColor(ImGuiCol_Button, ImVec4(0, 0, 0, 0));
    bool open = BeginCombo("##v", NULL, NULL, 180, ImGuiComboFlags_NoPreview | ImGuiComboFlags_HeightLargest);
    PopStyleColor(2);
